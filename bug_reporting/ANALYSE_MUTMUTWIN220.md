...
**Review-Stand:** 2026-08-31
**Arbeitsbaum:** `<repository root>`
**Git-Basis:** Branch `fix/360-review-hardening`, frisch von `main` bei `6cb727d`
**Status:** drei adversariale Review-Wellen plus finaler Multi-Agenten-Diff-/Claim-Reaudit, unabhängiger pytest-/Execution-Basis-Reaudit und adversarialer Finalgate-/Dogfood-/Security-Reaudit durchgeführt; 111 Befunde (30 P0, 53 P1, 28 P2; MW220-001 bis -111) lokal implementiert und gezielt beziehungsweise deterministisch belegt; vollständige Post-MW220-111-Testsuite, Dependency-Audit, finaler Dogfood-Recheck, kanonisches Semgrep-Gate, reproduzierbarer Build und Git-owned Kandidateninventar sind grün; der GitHub-CI-Lauf wird auf ausdrückliche Nutzeranweisung wegen des Billing-Problems übersprungen und nicht als PASS gewertet
...
Dieses Urteil beschreibt ausschließlich den ursprünglichen Ausgangssnapshot. Die zweite Review-Welle steht in Abschnitt 14, ihr Closure-Stand in Abschnitt 15, die dritte Review-Welle einschließlich des finalen Diff-/Claim- und pytest-/Execution-Basis-Reaudits in Abschnitt 16, deren Umsetzungsstand in Abschnitt 17 und die noch ausstehenden Gesamtgates des finalen Git-Clones in Abschnitt 18.
...
- historische vollständige Ausgangsgates und gezielte Regressionen der Fixwellen; die vollständigen Test-, Coverage-, Lint-, Typ-, Build-, Dependency-, Semgrep- und Dogfooding-Gates des finalen Baums bleiben Abschnitt 18 vorbehalten; Remote-CI ist dort als ausdrücklich akzeptierte Billing-Ausnahme, nicht als PASS, dokumentiert;
- Abgleich mit aktueller offizieller Ruff-, Hatch- und Upstream-mutmut-Dokumentation.

Die erste Review- und Fixwelle lief in einem lokalen ZIP-Snapshot ohne `.git`. Seit der zweiten Welle ist ausschließlich der frische Clone unter `<repository root>` auf `fix/360-review-hardening`, abgezweigt von `main` bei `6cb727d`, autoritativ und bearbeitet. Die Migration aus dem lokalen Legacy-ZIP-Baum wurde byte- und inventarbasiert bestätigt; dort verblieben keine exklusiven relevanten Dateien. Der Legacy-Baum ist keine zweite Source of Truth.

Das Publizieren des GitHub-Branches, die Integration und das GitHub-Release sind autorisiert, aber noch nicht erfolgt. Der Nutzer hat nach Live-Prüfung der bereits existierenden Tags `v2.5.0`, `v2.5.1` und `v2.20.0` ausdrücklich `v2.21.0` festgelegt. Projektmetadaten, Lock, Installationshinweise und installierte CLI melden konsistent `2.21.0`; Tag und Release werden erst auf dem integrierten, lokal vollständig verifizierten Commit erzeugt.
...
| MW220-069 | P1 | Python-Metadaten behaupten einen unbeschränkten `>=3.12`-Runtimevertrag, obwohl private Containmentbackends nur für CPython 3.12–3.14 auditiert sind | statischer Metadatenvertrag; lokal implementiert; Remote-CI wegen Billing übersprungen, kein PASS |
| MW220-070 | P1 | Das GitHub-Repository besitzt keine belastbare Windows/Linux- und CPython-3.12–3.14-CI-Matrix mit getrennten Lock-, Quality-, Test- und Auditgates; globale Frozen-Konfiguration und ein einplattformiges Audit können dabei false-green werden | Workflow-/Lock-/Audit-Vertrag; lokal implementiert; Remote-CI wegen Billing übersprungen, kein PASS |
| MW220-071 | P1 | GitHub-Release-Evidenz ist nicht durch SHA-gepinnte Actions sowie einen gelockten Build, Upload und installierte Wheel-/Sdist-Smokes abgesichert | Supply-Chain-/Artefaktvertrag; lokal implementiert; Remote-CI wegen Billing übersprungen, kein PASS |
...
Für diesen Korrekturstand hat der Nutzer den Remote-CI-Nachweis wegen eines bekannten GitHub-Billing-Problems ausdrücklich abgewählt und die daraus verbleibende Unsicherheit akzeptiert. Das ist eine dokumentierte Gate-Ausnahme, kein CI-PASS; die lokale Matrix bleibt vollständig verpflichtend.
...
| MW220-027 | implementiert | Ruff verwendet `extend-exclude` statt ersetzender Excludes; `pyproject.toml`; repositoryweites Finalgate bleibt ausstehend |
...
| MW220-031 | implementiert | README und Reviewdokumente bilden die Vertrauensgrenzen ab; die abschließende Dokument-/Artefaktabstimmung und Gesamtgates bleiben offen |
...
| MW220-047 | geschlossen; Kontextvollständigkeit durch MW220-073 erweitert | Mapping bleibt korrektheitshalber nichtautoritativ/Vollsuite; Vollsuite-Verdicts dürfen nur bei identischem beweisbar vollständigem Source-/Test-/Config-/Dependency-/Environment-/Import-/Runtimekontext wiederverwendet werden; `stats.py`, `orchestrator.py`; `test_result_reuse_119.py`, `test_dependency_basis_220.py` |
...
|---|---|---|
| MW220-050 | implementiert; Finalgate ausstehend | `process/run_lock.py`, `orchestrator.py`, `cli.py`; `test_run_lock.py`, `test_run_lock_processes.py`, `test_run_surface_integration_220.py` |
| MW220-051 | implementiert; Finalgate ausstehend | CI-Export ohne Legacy-Autorität und mit Live-Basis; `test_cicd_export_requires_live_source_test_and_config_basis`, `test_modern_completed_run_without_basis_cannot_authorize_cicd_export` |
| MW220-052 | implementiert; Finalgate ausstehend | stabiler Basis-Doppelsnapshot plus Pre-Completion-Revalidierung; `test_initial_run_basis_must_be_a_stable_snapshot`, `test_mid_run_source_drift_is_recorded_failed_not_completed` |
| MW220-053 | implementiert; Finalgate ausstehend | CLI-Konfigurationsreihenfolge; `test_since_commit_uses_effective_custom_tests_dir_before_filtering` |
| MW220-054 | implementiert; Finalgate ausstehend | `atomic_file.py`, `models.py`, `mutant_diff.py`; `test_atomic_write_safety_220.py` |
| MW220-055 | implementiert; Finalgate ausstehend | namespace-sichere Runner-/Stats-Sidecars; `test_runner_sidecar_safety.py` |
| MW220-056 | implementiert; Finalgate ausstehend | `generated_hash` bindet Meta an Ausgabebytes; erste drei Regressionen in `test_staging_authority_p1_220.py` |
| MW220-057 | implementiert; Finalgate ausstehend | descriptorgebundene `_copy_with_retry()`-Publikation; `test_atomic_staging_copy_detects_close_substitute_without_external_write` |
| MW220-058 | implementiert; Finalgate ausstehend | `browser.py`, `cli.py`; `test_browser_evidence_invalidation_220.py`, `test_results_marks_invalidated_evidence_not_release_ready` |
| MW220-059 | implementiert, fokussiert verifiziert; Finalgate ausstehend | `db.py` exklusive Voranlage sowie Connect-/Sidecar-/Exit-/Pre-Return-Identitätsprüfungen; Hardlink-/Swap-Regressionen einschließlich `test_load_current_run_rejects_permanent_path_swap_after_final_query` |
| MW220-060 | implementiert, fokussiert verifiziert; Finalgate ausstehend | `db.py` domänenspezifische Korruptionsgrenze, Completed-Invarianten, `plan_digest`, Ordinal-, High-Water- und Orphanprüfung; Planmanipulations-/Header-Rollback-/stale-Artifact-Regressionen in `test_db_state_boundary_220.py` |
| MW220-061 | implementiert, fokussiert verifiziert; Finalgate ausstehend | `atomic_spawn.py`; `test_atomic_spawn.py`, Windows-Hard-Exit-Test in `test_pool_shutdown.py` |
| MW220-062 | implementiert, fokussiert verifiziert; Finalgate ausstehend | seekbarer Windows-Bootstrap in `suspended_spawn.py`; `test_wedged_sitecustomize_large_payload_is_contained_and_nonblocking` |
| MW220-063 | implementiert, fokussiert verifiziert; Finalgate ausstehend | Runtime-/Resume-Fail-Closed; `test_suspended_spawn.py`, `test_process_worker.py` |
| MW220-064 | implementiert, fokussiert verifiziert; Finalgate ausstehend | POSIX-Session vor Interpreterstart; `posix_spawn.py`, `test_generation_supervisor.py` |
| MW220-065 | implementiert, fokussiert verifiziert; Finalgate ausstehend | executorbesessene pytest-PGID; `test_worker_sigkill_reaps_registered_pytest_session` |
| MW220-066 | implementiert, fokussiert verifiziert; Finalgate ausstehend | EOF-sicheres Pre-exec-Pipe-Gate mit synchroner Worker-/PGID-Publikation; realer WSL-SIGKILL-Test in `test_pool_shutdown.py` |
| MW220-067 | implementiert, fokussiert verifiziert; Finalgate ausstehend | unabhängige Executor- und Typechecker-Cleanup-Stufen; `test_cleanup_failures_do_not_skip_later_resources_or_escape`, `test_job_close_failure_does_not_skip_fallback_kill_or_mask_timeout` |
| MW220-068 | implementiert, fokussiert verifiziert; Finalgate ausstehend | eigener DONE-Join; `test_generation_supervisor_teardown.py` |
| MW220-069 | implementiert, fokussiert verifiziert; Remote-CI Billing-bedingt übersprungen, kein PASS | `requires-python`-/CPython-Vertrag und `test_python_metadata_matches_fail_closed_runtime_support` |
| MW220-070 | implementiert, fokussiert verifiziert; Remote-CI Billing-bedingt übersprungen, kein PASS | Windows-/Ubuntu- und CPython-3.12–3.14-Matrix, echte `uv lock --check`-Driftprüfung und matrixweites Audit; `test_ci_covers_supported_matrix_and_separate_release_gates` |
| MW220-071 | implementiert, fokussiert verifiziert; Remote-CI Billing-bedingt übersprungen, kein PASS | SHA-gepinnte Actions, gelockter reproduzierbarer Offline-Build und installierte Wheel-/Sdist-Smokes; `test_ci_uses_only_full_sha_pinned_allowlisted_actions` |
...
| MW220-073 | implementiert und unabhängig fokussiert reauditiert; Finalgate ausstehend | typisierte Vollständigkeit der Dependency-/Environment-/Import-/Runtimebasis, Reuse-Sperre und CI-Ablehnung bei unvollständiger Evidenz; `stats.py`, `orchestrator.py`, `cli.py`, `test_dependency_basis_220.py` |
| MW220-074 | implementiert, fokussiert verifiziert; Finalgate ausstehend | restliche Stats-/CI-/Plugin-/Guard-/Proof-/Argfile-/Run-Lock-Publisher auf `atomic_file.py` migriert; geschlossenes Writer-Inventar und Parent-Swap-Regressionen |
| MW220-075 | implementiert, fokussiert verifiziert; Finalgate ausstehend | Linux-`PR_SET_PDEATHSIG` plus Parent-PID-Racecheck am privaten Child-Entry und EOF-/`killpg`-Fallback; reale Hard-Parent-Kind-/Baumtests |
| MW220-076 | implementiert und unabhängig fokussiert reauditiert; Finalgate ausstehend | generisches `type_check_command` erzwingt typisierte Basisunvollständigkeit, sperrt Reuse/Score-/CI-Autorität; `stats.py`, `orchestrator.py`, `test_dependency_basis_220.py` |
| MW220-077 | implementiert und durch zwei unabhängige Red-Team-Reaudits fokussiert verifiziert; lokales Finalgate wird erneut ausgeführt, Remote-CI Billing-bedingt übersprungen | pytest-Boundary Schema v2, Root-/Config-/Target-Identitäten, option-safe Argfile, früher Conftest-Guard, Worker-/Executor-Fatalität, pytest-8.2/9-Realprozesse; `pytest_boundary.py`, `runner.py`, `process/executor.py`, `process/worker.py`, Boundary-/Surface-Regressionen |
| MW220-078 | implementiert und fokussiert verifiziert; Finalgate ausstehend | zentrale Erkennung historischer generischer-Checker-Runs und explizites `release-ready: no` in CLI/Browser; `db.py`, `cli.py`, `browser.py`, Anzeige-Regressionen |
| MW220-079 | implementiert und Workflow-Reaudit grün; Remote-CI Billing-bedingt übersprungen | getrennte `SOURCE_DATE_EPOCH`-Zuweisung/Export; actionlint mit ShellCheck/Pyflakes sowie Zizmor ohne Findings |
...
| MW220-085 | implementiert und in einer wirklich frischen externen Umgebung verifiziert; Remote-CI Billing-bedingt übersprungen | fest gepinnte `hatchling`-/`editables`-Buildgruppe, expliziter Bootstrap vor jedem CI-`--no-build-isolation`-Sync und statischer Reihenfolgevertrag in `test_release_supply_chain.py` |
...
Die implementierten Befunde besitzen gezielte Regressionen oder deterministische Byte-/Prozessbelege. Der vollständige Abschlusslauf nach MW220-090 bis -104 und dem Versionsbump bestand strikt mit 1.994 Tests und 44 Skips; der ebenso vollständige Coverage-Lauf bestand mit 85 % (8.513 Statements, 1.271 Missing). MW220-105 wurde danach durch 2/2 fokussierte Prozessfälle und 50/50 Stresswiederholungen geschlossen. MW220-106 bis -110 härten Checkoutbytes, Hashprovenienz, Live-State, Installationsautorität und den Standalone-Harness; ihr damaliger Supply-Chain-Cluster bestand mit 10/10 Tests. Nach Ende eines fremden CPU-sättigenden Mutationslaufs bestanden 20/20 Wiederholungen des IL-Integrationstests und zwei vollständige strikte Läufe mit jeweils 1.999 Tests und 44 Skips. Der vollständige Post-MW220-111-Kandidatenlauf bestand anschließend mit 2.000 Tests und 44 Skips. Sein erster Vor-/Nachvergleich erzeugte ausschließlich zwei Pytest-Assertion-Rewrite-Caches von Hypothesis; der unmittelbare warme Wiederholungslauf bestand erneut mit 2.000 Tests und 44 Skips und hielt `.venv` exakt bei 11.597 Dateien, 484.925.395 Bytes und SHA-256 `95c1fed54da9cbdfdd8ab7d7addec3b63deb11b88e92d85968d2cb42e3488d82`. Der anschließende Dogfood-Recheck auf demselben Kandidaten bestand mit 86/90 killed, vier äquivalenten Survivors, keinen Timeout-/Suspicious-/Skipped-/No-tests-/Unchecked-Fällen und 95,6 %. Sein CI-Export war vor und nach dem absichtlichen Environment-Drift-Test byteidentisch; die abweichende Basis wurde mit Exit 1 und ohne stale Artefakt abgelehnt. Das anschließende Semgrep-Gate bestand auf dem eingefrorenen Source-Scope vollständig und fail-closed. Der aus dem kanonischen Git-Index exportierte Kandidat bestand danach das unabhängige Inventar, Gitleaks, Workflow-Gates und einen byteidentischen Offline-Doppelbuild samt Archiv-, Lizenz-, Metadaten- und Laufzeitsmokes. Die contentionbedingte frühere Busy-Loop-Fehlklassifikation ist damit weder Produktbug noch PASS. Der lokale Kandidat ist commit- und publikationsbereit; der integrierte Releasecommit wird vor Tag und Release nochmals artefaktgebaut.

| Abschlussgate | Status |
|---|---|
| vollständige Testsuite | Post-MW220-111-Kandidatenbaum zweimal vollständig grün: jeweils 2.000 bestanden, 44 übersprungen, striktes Thread-Warnungsgate; zusätzlich 20/20 frühere IL-Wiederholungen grün |
| Coverage | 1.994 bestanden, 44 übersprungen, 85 %, 8.513 Statements / 1.271 Missing; MW220-105 bis -111 ändern keine Produktionsstatements |
| Ruff Check und Format | Post-MW220-111 repositoryweit grün; Ruff Check ohne Befund, Ruff Format bestätigt 168 formatierte Python-Dateien |
| mypy und Import Linter | 38 mypy-Dateien sowie 37 Dateien / 123 Importabhängigkeiten / 1 Vertrag grün |
| Lockprüfung und Dependency Audit | `2.21.0`-Lock mit 115 Paketen grün; finaler pfadunabhängiger Dependency-Body 7.345 Bytes / SHA-256 `7deef5fd17ec5e7aa60cb18c6be87e3d1a8478af3d7e1262d0bb2155bbd1fdd6`; frischer leerer HTTP-Cache, keine bekannten Advisories |
| frischer Build und Artefaktinventar | aus dem gestagten Post-MW220-111-Git-Kandidaten zweimal byteidentisch offline gebaut: Wheel 270.613 Bytes / SHA-256 `8a89f0f6243ff7775ddcd918965b86f3385adda25adf051b8e1f5254d1e26e89`, Sdist 683.043 Bytes / SHA-256 `07a5e86969eacd888aee30751643ee87e3a3f5740b58ae263df7a6c2ec0f38b4`; 44/224 Archivmember, Lizenz-/Wrapper-/Policybytes, Twine 7.0.0 und Check-Wheel-Contents 0.6.3 grün; Wheel-Smokes auf CPython 3.12.13/3.13.13/3.14.7 und Sdist-Smoke auf 3.12.13 grün; integrierter Rebuild ausstehend |
| getrackter fail-closed Semgrep-Wrapper | finaler Post-MW220-111-Scope grün: 218 Manifestdateien (`45e0e74e02ef8d9fb9de4dd0a31455095890ce3d6c9ad7c11d3c5640e23a029c`), 168 Targets (`d54286198b9fca7419b0db6728cd5a48e67269d5f8cf9c8f2fdda1e163a92f8e`), 49 Policy-Skips, 1.074 Definitionen, 342 Rule-IDs, 20/20 allowgelistete Matches, 0 unerwartete Findings/Errors/Skipped-Rules/Fixpoint-Timeouts |
| E2E-Umgebungsisolation | vollständige Läufe und isolierte pytest-8.2.2-Grenze grün; Post-MW220-111-Warmlauf mit exakt identischem Vor-/Nachdigest: 11.597 Dateien / 484.925.395 Bytes / SHA-256 `95c1fed54da9cbdfdd8ab7d7addec3b63deb11b88e92d85968d2cb42e3488d82` |
| dokumentierter Dogfooding-Pilot auf dem Post-MW220-111-Kandidaten | 90 total, 86 killed, 4 äquivalente Survivors, 0 timeout/suspicious/skipped/no-tests/unchecked, 95,6 %, 105,4 s; identischer Export vor/nach Negativtest 288 Bytes, SHA-256 `3a01ee2d05da06ac61c6fb3a0e4dc4bb9c5cf89c2b2daa46e5ab55080c40cc7f`; Environment-Drift endet 1 und entfernt das stale Artefakt |
| Git-owned Kandidateninventar | 409 kanonische Indexdateien; 42 neu und 110 geändert; `git diff --cached --check`, Gitleaks über 279 Commits und Kandidatenbaum, Unicode-/Bidi-/NUL-/Control-/EOL-/Pfad-/Case-/Symlink-/Reparse-/Artefakt-/Merge-Marker- und unabhängiger Claim-Reaudit ohne Befund |
| Branch-/PR-Publikation | ausstehend |
| GitHub-CI auf dem finalen Commit | auf ausdrückliche Nutzeranweisung wegen Billing übersprungen; kein PASS |

**Aktuelles technisches Urteil: lokales Commit-/Publikations-GO, noch kein Release-Gesamt-GO.** Alle 111 Befunde sind implementiert und gezielt beziehungsweise deterministisch belegt; die vollständige Post-MW220-111-Suite, der Dependency-Audit, der finale Dogfood-Recheck, das kanonische Semgrep-Gate, der reproduzierbare Kandidatenbuild und der abschließende Git-owned-Inventarrecheck sind grün. Es folgen Commit, Push und PR-Integration; erst der danach verifizierte integrierte `main`-Commit darf den annotierten Tag und das GitHub-Release tragen. GitHub-CI wird wegen des vom Nutzer benannten Billing-Problems ausdrücklich nicht ausgeführt; diese akzeptierte Remote-Evidenzlücke ist kein grüner CI-Nachweis. `v2.21.0` ist live als frei und der Remote-`main` unverändert bei `6cb727d` geprüft. Der lokale Legacy-ZIP-Baum ist nicht autoritativ.
