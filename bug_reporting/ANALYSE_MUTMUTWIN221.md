...
**Status:** Die vollständigen lokalen Implementierungsgates einschließlich strikter Coverage-Suite,
...
|---|---|---|
| MW221-004, 8.3-/Alias-False-Positive | bestätigt | identitätsbasierte Pfadprüfung an Atomic-, DB-, Staging- und pytest-Grenzen implementiert und lokal verifiziert |
...
| MW221-004 | P1 | VERIFIED_FIXED | Windows-Aliase werden über Komponenten-/Objektidentität statt Textgleichheit geprüft; vier Konsumentenfamilien angepasst. Reale 8.3-Deckung bleibt volumenabhängig. |
| MW221-005 | P1 | VERIFIED_FIXED | Apply/Show arbeiten wieder von der öffentlichen Originaldefinition und erhalten Signatur, Annotationen, Defaults, Dekoratoren und führenden Kontext. |
| MW221-006 | P1 | VERIFIED_FIXED | Pragma-Scan behandelt `SyntaxError`-Unterklassen einschließlich `TabError`/`IndentationError` kontrolliert. |
| MW221-007 | P1 | VERIFIED_FIXED | Abgeleitetes Staging wird nicht selbst gehasht; der v2.21.0-Gesamtlauffehler besitzt eine Regression. |
...
| MW221-010 | P1 | VERIFIED_FIXED | Vollrun-Autorität wird persistiert; Browser-Retest, CLI-Subset und Legacy-Runs können keinen Vollscore exportieren. CLI `results` und Browser kennzeichnen den Subset-Scope, beziehen Total/Score ausdrücklich nur auf diese Population und weisen `release-ready: no` aus. |
| MW221-011 | P1 | VERIFIED_FIXED | `git diff --name-only -z` erhält nicht-ASCII-Dateinamen ohne C-Quoting-Verlust. |
...
| MW221-014 | P1 | VERIFIED_FIXED | Symlink-/Junction-Source-Layouts bleiben aus Sicherheitsgründen ungestagt, werden aber nicht mehr still verworfen, sondern diagnostiziert. |
...
| MW221-017 | P1 | VERIFIED_FIXED | Fake-`Popen`-Objekte dürfen keinen realen PID-/Tree-Sweep auslösen; gefährdete Tests mocken den Cleanup-Seam. |
| MW221-018 | P1 | VERIFIED_FIXED | E2E-Referenz verlangt mindestens einen echten Survivor und schließt Timeout/Suspicious als Ersatz-Kills aus. |
| MW221-019 | P1 | VERIFIED_FIXED | Governance prüft Struktur und Zustandsübergänge statt feste Bool-Werte, Branches, Commits oder Erfolgsprosa; LIVE-State folgt der in Abschnitt 8 belegten Releasephase. |
| MW221-020 | P2 | VERIFIED_FIXED | PEP-263-Encoding wird beim Lesen, Generieren, Anzeigen und Anwenden berücksichtigt; nicht darstellbare generierte Bytes wechseln kontrolliert auf UTF-8. |
| MW221-021 | P2 | VERIFIED_FIXED | `typing_extensions.cast` wird wie `typing.cast` als semantisch äquivalenter Wrapper behandelt. |
...
| MW221-023 | P2 | VERIFIED_FIXED | Methodenargumente werden ohne `None`-Sentinel vollständig weitergereicht. |
| MW221-024 | P2 | VERIFIED_FIXED | Parameterlose und `*args`-Staticmethods/Instanzmethoden werden nicht mehr pauschal ausgelassen. |
| MW221-025 | P2 | VERIFIED_FIXED | Quellnamen mit reserviertem `__mutmut_`-Delimiter werden reversibel in einen disjunkten Hex-Namensraum kodiert; Mapping und Runtime-Dispatch dekodieren dieselbe Identität. |
| MW221-026 | P2 | VERIFIED_FIXED | Regex-Quantifier-Mutation berücksichtigt Zeichenklassen; Validierung wird nicht von der öffentlichen `re`-Cache-Reihenfolge abhängig gemacht. |
| MW221-027 | P2 | VERIFIED_FIXED | Docstrings werden strukturell über Parent-Metadaten erkannt, unabhängig von Quote-Stil. |
| MW221-028 | P2 | VERIFIED_FIXED | Pragmaerkennung verlangt den exakten Kommentarvertrag und matcht keine Lookalikes wie `no mutated`. |
...
| MW221-030 | P2 | VERIFIED_FIXED | SQLite-Busy/Locked wird als konkurrierender Zugriff diagnostiziert und nicht mehr als Aufforderung zum Cachelöschen. |
| MW221-031 | P2 | VERIFIED_FIXED | Plan-Dublettenerkennung nutzt ein Set statt einer quadratischen Listensuche. |
| MW221-032 | P2 | VERIFIED_FIXED | `apply` löst den Mutantennamen vor jeder Evidenzinvalidierung auf; ein Tippfehler löscht kein CI-Artefakt. |
...
| MW221-035 | P2 | VERIFIED_FIXED | Dotenv-Dateien bleiben aus dem Staging ausgeschlossen, ihre Bytes binden aber den Einweg-Basisdigest. |
| MW221-036 | P2 | VERIFIED_FIXED | Generische Namen wie `build`, `dist`, `html`, `bug_reporting` und `_docs` werden nur am Workspace-Root pauschal ausgeschlossen, nicht in jedem Source-Unterbaum. |
| MW221-037 | P2 | VERIFIED_FIXED | Staging und Run-Basis schließen jetzt dieselbe vollständige SQLite-Namespace-Familie aus: DB, `-journal`, `-wal` und `-shm`. |
...
| MW221-041 | P2 | VERIFIED_FIXED | Config-Tokenisierung deaktiviert `shlex`-Kommentartrunkierung; `#` bleibt Bestandteil valider Optionen/Pfade. |
| MW221-042 | P2 | VERIFIED_FIXED | `tests_dir`-Node-IDs werden vor der `--since-commit`-Filterung auf den Dateipfad normalisiert. |
| MW221-043 | P2 | VERIFIED_FIXED | Ein gültiger Diff ohne geänderte Mutationstargets ist jetzt ein diagnostizierter Exit-0-No-op; ungültige Git-Refs bleiben Exit 2 und JSON-Ausgabe bleibt maschinenlesbar. |
...
`VERIFIED_FIXED`, 7 `ACCEPTED_LIMITATION`, 3 `REJECTED_AS_BUG`, 4
`OUT_OF_TARGET`, 4 `MIXED` und 1 `OPEN_PROCESS`.
Die 70 Codex-Follow-ups werden getrennt von diesen 46 MW-Zeilen gezählt.
...
Die folgenden Zeilen bewahren die historischen Läufe einschließlich ihrer damals
offenen Gates und negativen Ergebnisse. Der aktuelle Abschlussstand und seine
Commit-/Tree-Evidenz stehen in Abschnitt 8.
...
| v2.21.1 vollständige Implementierungssuite auf `19f8d3a` | Commit `19f8d3a0006ca9a91125b521d097281c92493e9a`, Tree `3adac8fd794f793b84ef57aef420351582175cf2`: 4 failed/2.430 passed/43 skipped in 2.109,49 s; 9.752 Statements, 1.453 Missing, 85 % Coverage; anschließend Checkout sauber und zugehörige Prozesse beendet | FAIL: ausschließlich vier Parameterfälle von `test_test_selection_args_reach_every_parent_phase` erwarteten nach CX221-070 weiterhin direkte Targets in argv; der bestehende Test wurde auf Inhalt und Lifetime der tatsächlichen Argfile angepasst. Kein neuer Produktbefund; keine Kandidatenfreigabe |
| v2.21.1 CX221-070 Anpassung der bestehenden Oberflächenregression | Alle bisherigen Selektions-/Config-/Rootprüfungen erhalten; exakte Argfilebytes an der Popen-Grenze und Cleanup nach jeder Elternphase geprüft. Agent-Verbund 158 passed in 30,26 s; unabhängiger Root-Verbund aus vier Fällen und Supply-Chain-Tests 61 passed in 3,39 s; Ruff/Format ohne Befund | fokussierte Testkorrektur ohne Produktänderung; vollständige commit-genaue Revalidierung bleibt erforderlich |
| v2.21.1 kanonischer Dogfood auf `19f8d3a` | 91/91 abgeschlossen, 90 killed/1 Windows-äquivalenter Survivor, alle Problem-Buckets 0, un-aborted, vollständige Basis, evidence_invalidated=0, 91/91 Run- und History-Fingerprints autorisiert; 125,8285 s Mutation/162,4083 s Wallclock; Source-/Envbytes unverändert und keine Restprozesse | vollständig abgeschlossener Scope-Nachweis auf diesem Commit. Der externe Nachprüfer hatte zunächst eine falsche leere Standard-Kopierliste erwartet; korrigierte quellenbelegte Auswertung desselben unveränderten Laufs bestand ohne erneuten Mutationslauf |
| v2.21.1 Candidate-Dokumentfehler auf `45cd62e` | Commit `45cd62ef489a49cd8889283d893218a9d8ee1121`, Tree `bda15ab5f0c594f71f88a494d9f88ee0bcca7064`: Die Kopftransformation entfernte die bestehende Roadmap-Vertragsphrase zur 70er-Codex-ID-Menge. Der gezielte Governancecluster reproduzierte 1 failed/64 passed in 3,61 s. Die überholte Vollsuite wurde nach ihrer sichtbaren Fehlermeldung kontrolliert abgebrochen; danach Checkout sauber und keine zugehörigen Testprozesse. | ABORTED, kein abgeschlossener Candidate-Nachweis. Der Previewvergleich übersah den Pflichtanker; die wahre Kopfzeile ist wiederhergestellt und auch für Housekeeping erhalten. Keine Produkt-/Teständerung und kein neuer Befund; die korrigierten Dokumentbytes benötigen eigene vollständige Kandidatengates |
...
## 8. Aktueller Release- und Evidenzstand
