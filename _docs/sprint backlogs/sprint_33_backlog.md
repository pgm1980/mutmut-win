...
|---|-------|-----|-------|--------|----|-----------|-------------|--------|
| 1 | [#105](https://github.com/pgm1980/mutmut-win/issues/105) | Bug | DOG-001: additiver Startup-Sockel im Timeout-Modell (gemessen, geclamps, transparent) | Dogfooding S2 | 5 | Must | 1 | ✅ `28a718f` |
| 2 | [#106](https://github.com/pgm1980/mutmut-win/issues/106) | Bug | QX-007: `no tests`-Producer — ungemappte Mutanten laufen heute die Vollsuite | Audit S3 (Laufzeit-Kern) | 5 | Must | 2 | ✅ `6430d18` |
| 3 | [#110](https://github.com/pgm1980/mutmut-win/issues/110) | Bug | Hygiene-Kleinkram: QX-017/018 (max_stack_depth-Falle!), QX-019-Rest, DOG-002 | Audit/Dogfooding S4 | 3 | Should | 3 (QX-018 gehört laufzeitlich zu #105/#106) | ✅ `ce907d0` |
| — | — | Gate | **Dogfooding-Re-Run (Zwischengate): brutto ≥ 80 % erwartet** | — | — | 4 | ✅ **86,9 % brutto** (212/244, 0 Timeouts) |
| 4 | [#107](https://github.com/pgm1980/mutmut-win/issues/107) | Bug | QX-001(+QX-020): Trampolin-Importkette → Kernel-Modul; Architektur-Skip fliegt | Audit S2 + Sprint-32-Marker | 8 | Must | 5 | ✅ `4659d36` |
| 5 | [#108](https://github.com/pgm1980/mutmut-win/issues/108) | Bug | UI-007: Browser-Diff = `show`-Diff (Single Source); DB-Fallback-Namensform | Audit S2 | 5 | Should | 6 | ✅ `fbed4cd` |
| 6 | [#109](https://github.com/pgm1980/mutmut-win/issues/109) | Bug | Browser/CLI-Robustheit: UI-010/011/013/016 | Audit S3/S4 | 5 | Should | 7 | ✅ `4b87423` |
| — | — | Gate | Abschluss-Dogfooding (gleicher Pilot, Vergleichbarkeit) + Score-Doku | — | — | 8 | ✅ **86,9 % brutto**, 158,9s, Architektur-Gate im Artefakt KEPT |
...
| Task | Beschreibung | Status |
|------|-------------|--------|
| A.1 | Abschluss-Pilot (gleiche Module) — Score dokumentiert; neue Funde → Pool | ✅ (keine neuen Funde; 1 Bedienungs-Lektion s. Protokoll) |
| A.2 | Maintenance-Backlog aktualisieren (erledigte Einträge raus, neue rein) | ✅ (13 Einträge raus, Pool = 13) |

---

## Dogfooding-Protokoll Sprint 33

Pilot identisch zu Sprint 32 (Vergleichbarkeit): `code_coverage.py` +
`type_checking.py`, jeweils `--force --no-progress`.

| Lauf | Zeitpunkt | Total | Killed | Survived | Timeout | Score brutto | Dauer |
|------|-----------|-------|--------|----------|---------|--------------|-------|
| Sprint-32-Referenz (Pilot 5) | v2.10.0 | 244 | 59 | 10 | **175** | **24,2 %** | (Timeout-dominiert) |
| **Zwischengate** (nach #105+#106+#110) | v2.11.0-dev | 244 | 212 | 32 | **0** | **86,9 %** | — |
| **Abschluss** (nach #107–#109, frisches Artefakt) | v2.11.0-dev | 244 | 212 | 32 | **0** | **86,9 %** | 158,9 s |

- **Sprint-Messziel erreicht:** brutto ≥ 80 % → 86,9 % (Zwischengate UND
  Abschluss, deterministisch identisch). Die 175 Pseudo-Timeouts sind
  vollständig verschwunden; alle 244 Mutanten sind bewertbar.
- **Timeout-Modell live:** Zwischengate Sockel 10,4s (clean 31,4s − Tests
  21,0s), Abschluss 12,2s (36,5s − 24,3s) — selbstkalibrierend je Lauf.
- **DOG-002 live:** IL-Hint erscheint exakt einmal pro Lauf (orchestratorseitig).
- **UI-016 live:** Die End-Summary erscheint im Abschluss-Lauf TROTZ
  `--no-progress` (im Zwischengate-Log fehlte sie noch — der Fix kam mit #109).
- **QX-001-Beweis:** Abschluss-Lauf mit `--force` = frisch generiertes
  Artefakt; der Clean-Run führte die Suite (840 Tests) INKLUSIVE des
  ungeskippten `test_import_linter_contracts_hold` im Trampolin-Artefakt
  aus — grün. Das Layer-Gate gilt wieder überall.
- **Survivors (32, dokumentiert):** ~18× `gather_coverage` (Fehlermeldungs-
  Texte, Pfad-Normalisierungs-Randfälle), ~14× `type_checking`-Parser
  (Report-Feld-Toleranzen). Ehrliche Test-Lücken, im Pool-Kontext S4 —
  keine neuen Pool-Einträge nötig (bestehende Module, bekannte Klasse).
- **Bedienungs-Lektion (kein Code-Fund):** `--paths-to-mutate` ist
  repeatable — zwei Pfade hinter EINEM Flag werden als Mutant-Name-Filter
  geparst („No mutants match the given names"). Korrekt: Flag wiederholen.
...
|------|--------|----------|
| Tests | `uv run pytest` | ✅ 868 passed, 4 skipped (Plan: ≥ 840) |
| Linting | `uv run ruff check src/ tests/` | ✅ 0 Findings |
| Format | `uv run ruff format --check src/ tests/` | ✅ 0 zu formatieren |
| Type Check | `uv run mypy src/mutmut_win/` | ✅ 20 Errors = Baseline, 0 neue |
| Security | `semgrep scan --config auto src/mutmut_win/ tests/` | ✅ 0 Findings (voller Sweep) |
| Architecture | lint-imports in-suite | ✅ KEPT — **auch im Trampolin-Artefakt (Skip entfernt)** |
| Mutation | Dogfooding-Pilot | ✅ **86,9 % brutto** (Ziel ≥ 80 %) |
