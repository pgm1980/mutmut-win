...
| H-04 | PostToolUse | **ROOT CAUSE + FIX**: Zwei Ebenen nötig: `"matcher": "Bash"` (Tool-Name) + `"if": "Bash(*git commit*)"` (Befehlsinhalt, Permission Rule Syntax). WICHTIG: Führender `*` in `Bash(*git commit*)` ist Pflicht weil Befehle mit `cd ...&&` beginnen. **VERIFIZIERT**: B-glob-if feuert nur bei git commit, nicht bei git add. | FIXED + VERIFIED |
